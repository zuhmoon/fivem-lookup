...
Setup, deployment, API behaviour and the reasoning behind the odd-looking bits. For the
short version aimed at anyone else, see [README.md](README.md).

Everything here was measured against the live Cfx API, not assumed — where a number appears
(38% of servers expose an anticheat name, 34% hide their player list), it came from a scan
recorded at the time. Cfx can change any of it without notice.
...
work. Players, resources and name-watching will be intermittent — that's step 2.
...
1. Sign up at [workers.cloudflare.com](https://workers.cloudflare.com) — the free tier's
   100,000 requests/day is far more than this uses.
...
- **No anticheat filter, deliberately.** One existed and was removed. Resource lists live
  only behind `single/<code>`, so filtering by anticheat meant a request per server; Cfx
  429s those bursts and answers with an HTML error page. Measured on the live site: 60
  servers at six-way concurrency landed 30, and 60 paced at 220ms landed 37. The feature was
  unreliable by nature, not by bug. Per-server detection on the detail panel stays — that's
  one request on a server you already opened. Don't rebuild the filter without a cache that
  survives longer than a session.
